...
adminip=$( curl -sS https://raw.githubusercontent.com/xe3r0/allow/main/accessmenu | grep $MYIP )
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/xe3r0/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
git config --global user.name "xe3r0"
git clone https://github.com/xe3r0/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/xe3r0/allow.git
git push -f https://${tokengit}@github.com/xe3r0/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/xe3r0/backdoor/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
clear
